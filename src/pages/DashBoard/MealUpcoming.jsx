...
const AddUpcomingMealModal = ({ onClose, onMealAdded }) => {
  ...
  const onSubmit = async (formData) => {
    ...
    try {
      const res = await fetch(`${BACKEND_URL}/api/upcomingMeals`, {
        method: "POST",
      ...
      if (res.ok) {
        ...
        setImageUrl("");
        onClose();
        onMealAdded();  // Notify parent to refresh list
      } else {
  ...
  return (
    ...
    >
      <div
        className="bg-[#810000] text-white w-full max-w-lg p-8 rounded-2xl relative"
      ...
      >
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          />
          {errors.title && (
            <p className="text-yellow-400 text-sm">{errors.title.message}</p>
          )}
          ...
          {errors.category && (
            <p className="text-yellow-400 text-sm">{errors.category.message}</p>
          )}
          ...
          />
          {errors.ingredients && (
            <p className="text-yellow-400 text-sm">{errors.ingredients.message}</p>
          )}
          ...
          />
          {errors.description && (
            <p className="text-yellow-400 text-sm">{errors.description.message}</p>
          )}
          ...
          />
          {errors.price && (
            <p className="text-yellow-400 text-sm">{errors.price.message}</p>
          )}
...
const MealUpcoming = () => {
  ...
  // Fetch upcoming meals from backend API
  const fetchUpcomingMeals = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${BACKEND_URL}/api/upcomingMeals?sort=likes`);
  ...
  // Publish a meal (move to all meals with post time)
  const handlePublish = async (mealId) => {
    try {
      const res = await fetch(`${BACKEND_URL}/api/upcomingMeals/${mealId}/publish`, {
        method: "PATCH",
      });
      ...
      Swal.fire(
        "Success",
        `Meal published to All Meals at ${formattedTime}!`,
        "success"
      );
    } catch (error) {
      console.error(error);
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 sm:p-6 jost-font bg-white rounded-2xl shadow-md">
      <h2 className="text-2xl sm:text-3xl font-semibold mb-6 text-center text-[#810000]">
        Upcoming Meals
      </h2>
      ...
      <div className="flex justify-end mb-4">
        <button
          ...
          className="flex items-center gap-2 bg-[#810000] text-white px-3 sm:px-4 py-2 rounded-lg hover:bg-[#a30000] text-sm sm:text-base"
        >
          ...
          <table className="min-w-full border-collapse border border-gray-300 rounded-lg overflow-hidden text-sm sm:text-base">
            ...
            <tbody>
              {meals.map(({ _id, title, likes, category }) => (
                <tr
                  key={_id}
                  className="border-b border-red-800 hover:bg-gray-50 transition"
                >
                  ...
                  <td className="p-2 sm:p-3 text-center">
                    <button
                      ...
                      className="flex items-center justify-center gap-2 bg-[#810000] text-white px-3 sm:px-4 py-2 rounded-lg hover:bg-[#a30000] text-xs sm:text-base"
                    >
                      <FaArrowUp /> Publish
                    </button>
                  </td>
      ...
      {showModal && (
        <AddUpcomingMealModal
          ...
          onMealAdded={fetchUpcomingMeals}
        />
      )}
    </div>
  );
